...
def test_freehold_indivisible_only_in_first_row():
    """Инвойс, комиссия перевода и «дойдёт застройщику» описывают единственный
    SWIFT — во второй строке прочерк «—» (пустая ячейка читалась бы как
    «данные не подставились»); ссылки на документы просто пустые."""
    rows = build_freehold_rows(_freehold())
    for idx in (7, 9, 10, 11, 12):
        assert rows[1][idx] == '—', f'колонка {idx} должна быть «—» во 2-й строке'
    for idx in (16, 17, 18):
...
def test_leasehold_indivisible_only_in_first_row():
    """Инвойс, курсы покупки/продажи и процент компании описывают одну отправку
    в MF Corp — во второй строке прочерк «—»; документы просто пустые."""
    rows = build_realty_rows(_leasehold())
    for idx in (6, 7, 8, 12):
        assert rows[1][idx] == '—', f'колонка {idx} должна быть «—» во 2-й строке'
    for idx in (21, 22, 23):
